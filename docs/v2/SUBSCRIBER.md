...
Implement `LaravelNats\Subscriber\Middleware\InboundMiddleware` and add the class name to `nats_basis.subscriber.middleware`. The package ships with `LogInboundMiddleware` (commented out by default) for debug logging via `Psr\Log\LoggerInterface`, **`CorrelationLogInboundMiddleware`** for correlation ids in shared log context ([CORRELATION.md](CORRELATION.md)), **`RedactedEnvelopeLogInboundMiddleware`** for debug logs with redacted envelope `data` ([OBSERVABILITY.md](OBSERVABILITY.md)), and **`IdempotencyInboundMiddleware`** for deduplication when **`nats_basis.idempotency.enabled`** is true ([IDEMPOTENCY.md](IDEMPOTENCY.md)).
...
- [FAQ](FAQ.md)
- [Roadmap](../ROADMAP.md) — v2.x themes including security (v2.6)
