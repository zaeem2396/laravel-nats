...
| **Header helpers + trace context (1.6.0+ / v2.7)** | `LaravelNats\Support\NatsHeaderBag` builds common headers and repeated values; optional `NATS_TRACE_CONTEXT_INJECT` copies valid W3C `traceparent` / `tracestate` from the active request. See [TRACE_CONTEXT.md](TRACE_CONTEXT.md). |
| **Connection selection (1.6.0+ / v2.7)** | `NATS_CONNECTION_SUBJECT_PREFIXES` maps subject prefixes to named connections, and `NatsV2::selectConnection()` exposes the selection result. Explicit connection arguments still win. See [CONNECTION_SELECTION.md](CONNECTION_SELECTION.md). |
| **Outbox recipe (1.6.0+ / v2.7)** | `NatsOutboxDispatcher` and `NatsV2::dispatchOutbox()` drain an app-provided `NatsOutboxStoreContract`; storage stays in your application. See [OUTBOX.md](OUTBOX.md). |
